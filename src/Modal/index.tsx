...
const Modal: FC<PropsWithChildren<ModalProps>> = (props) => {
  ...
  const ModalDOM = (
    <div ref={modalRef} className={rootClass} onKeyDown={handleKeyDown} style={{ zIndex, ...style }}>
      {mask && (
        <div className={`${selfPrefixCls}-mask`} style={maskStyle} onClick={maskClosable ? handleClose : undefined} />
      )}
      <div className={wapperClass}>
        <div className={`${selfPrefixCls}-container`} style={{ width, ...bodyStyle }}>
          {closable ? (
          ...
          {title ? <div className={`${selfPrefixCls}-header`}>{title}</div> : null}

          <div className={`${selfPrefixCls}-body`}>{children}</div>
          {footer ? (
            <div className={`${selfPrefixCls}-footer`}>{footer === true ? <DefaultFooter /> : footer}</div>
          ) : null}
        </div>
      </div>
    </div>
  ...
  const attach = typeof getContainer === 'function' ? getContainer() : getContainer;
