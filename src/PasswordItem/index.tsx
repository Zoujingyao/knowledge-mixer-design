...
import type { InputRef, FormItemProps, PopoverProps } from 'antd';
...
export const defaultRules = [
  { pattern: /^.{6,20}$/, message: '长度为6~20个字符' },
  {
    validator: (_: any, value: string) => {
      ...
      const matched =
        Number(letterRegex.test(value)) + Number(digitRegex.test(value)) + Number(specialCharRegex.test(value)) >= 2;
      if (!matched) {
...
export const validateCheckList = (rule: (typeof defaultRules)[number], value: string) => {
 ...
 */
function PasswordItem(props: PasswordFormItemProps) {
  const { rulesForCheckList = defaultRules, inputProps = {}, popoverProps = {}, rules = [], ...restProps } = props;
  ...
  const ValidateMessages = () => {
    const [messageList, setMessageList] = useState<{ message: string; matched: boolean }[]>([]);

    const ValidatedItem = ({ matched, msg }: { matched: boolean; msg: string }) => {
      return (
    ...
    useEffect(() => {
      ...
      const promises = rulesForCheckList.map(
        (r) => validateCheckList(r, inputValue || inputRef?.current?.input?.value || ''), // defaultValue
      );
    ...
    return (
      <div>
        {messageList.map((item) => {
          return <ValidatedItem matched={item.matched} msg={item.message} key={item.message} />;
        })}
      </div>
    );
  };
  ...
  return (
    <Popover content={<ValidateMessages />} trigger="focus" placement="right" {...popoverProps}>
      <Form.Item
        ...
        rules={[
          {
            validator: (_, v) => {
              const promises = rulesForCheckList.map((r) => validateCheckList(r, v || ''));
        ...
        {...restProps}
      >
        <Input.Password ref={inputRef} autoComplete="off" onChange={handleChange} {...restInputProps} />
      </Form.Item>
    </Popover>
  );
}
